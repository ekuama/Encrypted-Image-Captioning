...
def caption_image_beam_search(beam_size_, device, encoder, decoder, phase_image_path, amp_image_path, word_map,
                              robust):
    k = beam_size_
    infinite_pred = False
    ...
    encoder_out = encoder_out.expand(k, encoder_dim)  # (k, num_pixels, encoder_dim)
...
def caption_search(image_name, name, robust):
    ...
    if name == "ResNet50":
        checkpoint = 'BEST_ResNet50_nic.pth.tar'
    elif name == "ResNet101":
        checkpoint = 'LESS_ResNet101_nic.pth.tar'
    else:
        checkpoint = 'BEST_ResNeXt101_nic.pth.tar'
    ...
    beam_size = 3
    seq = caption_image_beam_search(beam_size, device_, encoder, decoder, phase_img, amp_img, word_map_, r)
