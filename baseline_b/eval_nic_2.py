...
from utils_nic_2 import get_eval_score


def evaluate(beam_size_, data_folder, data_name, device, encoder, decoder, word_map, vocab_size, rev_word_map):
    loader = torch.utils.data.DataLoader(CaptionDataset(data_folder, data_name, 'TEST', False),
...
if __name__ == '__main__':
    ...
    for name in cnn_names:
        checkpoint = name + '_nic_2.pth.tar'
        print(checkpoint)
        ...
        eval_metrics = evaluate(beam_size, data_folder_, data_name_, device_, encoder_, decoder_,
                                word_map_, vocab_size_, rev_word_map_)
        ...
        checkpoint = 'BEST_' + name + '_nic_2.pth.tar'
        print(checkpoint)
        ...
        checkpoint = 'LESS_' + name + '_nic_2.pth.tar'
        print(checkpoint)
