...
from model_rdn import EncoderA, DecoderWithAttention
from utils_rdn import AverageMeter, accuracy, clip_gradient, save_checkpoint, adjust_learning_rate
...
def main(arg):
    ...
    pos_loss = nn.MSELoss(reduction='sum')
    ...
    for epoch in range(start_epoch, arg.epochs):
        ...
        t_loss, t_acc = train(arg=arg, train_loader=train_loader, encoder=encoder, decoder=decoder, criterion=criterion,
                              model_optimizer=model_optimizer, epoch=epoch, device=device, pos_l=pos_loss)

        # One epoch's validation
        recent_bleu4, v_loss, v_acc = validate(arg=arg, val_loader=val_loader, encoder=encoder, decoder=decoder,
                                               criterion=criterion, device=device, word_map=word_map, pos_loss=pos_loss)
    ...
    with h5py.File(os.path.join('models/' + arg.cnn_name + '_rdn.hdf5'), 'a') as h:
...
def train(arg, train_loader, encoder, decoder, criterion, model_optimizer, epoch, device, pos_l):
    decoder.train()
    encoder.train()
    ...
    for i, (phase, amp, caps, caplens) in enumerate(train_loader):
        ...
        scores, caps_sorted, decode_lengths, sort_ind, pos, pred_pos = decoder(imgs, caps, caplens)
...
def validate(arg, val_loader, encoder, decoder, criterion, device, word_map, pos_loss):
    ...
    with torch.no_grad():
        ...
        for i, (phase, amp, caps, caplens, all_caps) in enumerate(val_loader):
            ...
            loss += (0.02 * pos_loss(pred_pos, pos))
...
if __name__ == '__main__':
    ...
    # Model parameters
    parser.add_argument('--emb_dim', type=int, help='dimension of word embeddings', default=1024)
    parser.add_argument('--decoder_dim', type=int, help='dimension of decoder RNN', default=1024)
    parser.add_argument('--attention_dim', type=int, help='dimension of decoder RNN', default=512)
